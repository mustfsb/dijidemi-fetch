...
export async function POST(request: NextRequest) {
    try {
        ...
        let body: { questionContext?: string; userNote?: string };
        try {
        ...
        if (!questionContext || typeof questionContext !== 'string') {
            return NextResponse.json({ error: 'questionContext gerekli' }, { status: 400 });
        }
        if (questionContext.length > 5000) {
            return NextResponse.json({ error: 'questionContext çok uzun' }, { status: 400 });
        }
        if (userNote && userNote.length > 5000) {
            return NextResponse.json({ error: 'userNote çok uzun' }, { status: 400 });
        }
