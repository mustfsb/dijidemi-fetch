...
export async function POST(request: NextRequest): Promise<NextResponse<AssignmentsResponse | { error: string }>> {
    try {
        ...
        if (baseCookie.includes('TEST_TOKEN')) {
             const mock = [
                { id: "1001", title: "Test Ödevi 1 (Matematik)", dateRange: "20.01.2026 - 25.01.2026", link: "mock_link_1" },
                { id: "1002", title: "Test Ödevi 2 (Fizik)", dateRange: "21.01.2026 - 26.01.2026", link: "mock_link_2" },
                { id: "1003", title: "Test Ödevi 3 (Kimya)", dateRange: "22.01.2026 - 27.01.2026", link: "mock_link_3" }
             ];
             // Upsert Mocks
             for (const a of mock) {
                  await supabase.from('homeworks').upsert({
                             ...
                             status: 'active'
                  }, { onConflict: 'homework_identifier' });
             }
             return NextResponse.json({ assignments: mock });
        }
        // -----------------------------
        ...
        // Upsert Real Assignments to Supabase
