...
async function verifyAdmin() {
  ...
  const supabaseSSR = createAdminClient();
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { error } = await supabaseServiceRole.from("users").update(updates).eq("id", id);
    if (error) throw error;
    ...
    if (updates.role) {
      ...
      if (adminUsername) {
        ...
        const { error: upsertError } = await supabaseServiceRole
          .from('admin')
          .upsert(
            { username: adminUsername, password: '', role: updates.role },
            { onConflict: 'username' }
          );
        if (upsertError) {
          console.error('[Admin Sync] Failed to upsert admin table:', upsertError);
        } else {
          console.log(`[Admin Sync] Set '${adminUsername}' role to '${updates.role}'`);
        }
      }
    }
