...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (trimmedUsername.length > 128 || trimmedPassword.length > 256) {
      return NextResponse.json({ error: 'Kullanıcı adı veya şifre çok uzun.' }, { status: 400 });
    }
    ...
    const storedPassword = typeof adminRecord.password === 'string' ? adminRecord.password.trim() : '';
    if (!storedPassword) {
      log(`[Auth] DENIED: admin user has no password configured`);
