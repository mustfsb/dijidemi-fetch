...
async function fetchImagePart(url: string, request: NextRequest, userId: string) {
    try {
        ...
        if (response instanceof NextResponse) throw new Error("Image proxy failed");
    ...
    if (context && context.length > 0) {
      for (const q of context) {
        ...
        if (resolvedImageUrl) {
            ...
            const part = await fetchImagePart(resolvedImageUrl, request, auth.userId);
