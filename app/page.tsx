...
import { Moon, Sun, ArrowLeft, Settings, LogIn, BookOpen, ClipboardList, ChevronRight, ChevronLeft, Calendar, Upload, Loader2, CheckCircle2, XCircle, Bot, Sparkles, Shield, Menu, X } from 'lucide-react';
...
export default function Home() {
    ...
    const assignments = useAssignments(showToast, auth.refreshCookies, auth.setIsLoggedIn);
    const books = useBooks();
    const testRunner = useTestRunner(showToast, program.schedule, program.setSchedule);
    ...
    const handleAskAIWrapped = (q: string) => {
        aiTutor.handleAskAI(q, testRunner.selectedTest, String(books.selectedBook?.id || ''));
    };
    ...
    const handleSendToPlayground = (video: any) => {
        ...
        if (!currentSelection.find(q => q.id === newQuestion.id)) {
            ...
            sessionStorage.setItem('playground_auto_select', JSON.stringify(currentSelection));
    ...
    // Derived Error
    const currentError = (activeTab === 'assignments' ? assignments.error :
        activeTab === 'books' && !testRunner.selectedTest ? books.bookError :
            testRunner.error);
    ...
    return (
        <div className="min-h-screen flex flex-col bg-background">
            ...
            {/* Header */}
            <header className={`header ${scrolled ? 'header-scrolled' : ''} ${mobileMenuOpen ? 'header-expanded' : ''}`}>
                            ...
                            <>
                                ...
                                >
                                    ...
                                    <Link href="/playground" className="nav-item group flex">
                                        ...
                                        <span className="font-mono ml-1">AI Öğretmen</span>
                                    </Link>
                                <button
                                    className="nav-btn"
                    ...
                    {mobileMenuOpen && (
                        ...
                        >
                            <div className="pt-4 pb-2 flex flex-col gap-2">
                                {auth.isLoggedIn ? (
                                    <>
                                        <button className={`mobile-nav-item ${activeTab === 'assignments' ? 'active' : ''}`} onClick={() => { setActiveTab('assignments'); setMobileMenuOpen(false); }}>
                                            <ClipboardList className="w-5 h-5 mr-3" />
                                            Ödevlerim
                                        </button>
                                        <button className={`mobile-nav-item ${activeTab === 'program' ? 'active' : ''}`} onClick={() => { setActiveTab('program'); setMobileMenuOpen(false); }}>
                                            <Calendar className="w-5 h-5 mr-3" />
                                            Program
                                        </button>
                                        <button className={`mobile-nav-item ${activeTab === 'books' ? 'active' : ''}`} onClick={() => { setActiveTab('books'); setMobileMenuOpen(false); }}>
                                            <BookOpen className="w-5 h-5 mr-3" />
                                            Kitaplar
                                        </button>
                                        <Link href="/playground" className="mobile-nav-item" onClick={() => setMobileMenuOpen(false)}>
                                            <Sparkles className="w-5 h-5 mr-3 text-red-500" />
                                            AI Öğretmen
                                        </Link>
                                        <button className="mobile-nav-item" onClick={() => { setShowSettings(true); setMobileMenuOpen(false); }}>
                                            <Settings className="w-5 h-5 mr-3" />
                                            Ayarlar
                                        </button>
                                    </>
                                ) : (
                                    <button className="mobile-nav-item" onClick={() => { auth.setShowLoginModal(true); setMobileMenuOpen(false); }}>
                                        <LogIn className="w-5 h-5 mr-3" />
                                        Giriş Yap
                                    </button>
                                )}
                                <div className="h-[1px] bg-border my-1" />
                                <button className="mobile-nav-item" onClick={() => { setTheme(theme === 'dark' ? 'light' : 'dark'); setMobileMenuOpen(false); }}>
            ...
            <main className="main">
                ...
                {activeTab === 'books' && !testRunner.selectedTest && (
                    !books.selectedSubject ? (
                        ...
                        <div>
                            <button className="back-btn" onClick={() => books.setSelectedSubject(null)}>
                        ...
                        <div>
                            <button className="back-btn" onClick={() => books.setSelectedBook(null)}>
                                <ArrowLeft className="w-4 h-4" />
                                Kitaplara Dön
                            </button>
                            ...
                            <div className="grid-cards">
                                {assignments.assignments.map(a => (
                                    <Card
                                        ...
                                        onClick={() => a.status !== 'deactive' && handleAssignmentClickWrapped(a)}
                                    >
                                        <CardHeader className="p-0 pb-2">
                                            <div className="flex justify-between items-center">
                                                <span className={`badge ${a.status === 'deactive' ? 'bg-zinc-800 text-zinc-500' : ''}`}>
                ...
                {activeTab === 'program' && !testRunner.selectedTest && (
                    <div className="space-y-6">
                        ...
                        {program.schedule && program.schedule.tasks && program.schedule.tasks.length > 0 ? (
                            <ProgramView
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        if (confirm('Tüm program ilerlemenizi sıfırlamak istediğinize emin misiniz?')) {
                                            const resetTasks = program.schedule!.tasks.map(t => ({ ...t, completed: false }));
                                            const resetSchedule = { ...program.schedule!, tasks: resetTasks };
                ...
                {/* Test View (Shared) */}
                {(testRunner.selectedTest || activeTab === 'test-view') && testRunner.selectedTest && (
                    <div>
                        {/* Toolbar */}
                        <div className="flex flex-col gap-4 mb-8 pb-6">
                            <button
                                ...
                                onClick={() => {
                                    testRunner.setSelectedTest(null);
                                    setActiveTab(auth.isLoggedIn && testRunner.assignmentContext ? 'assignments' : 'books');
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                <h1 className="text-2xl font-bold">{testRunner.selectedTest.name}</h1>
                                {auth.isLoggedIn && (
                                    <Button
                                        className="save-btn"
                                    ...
                                    >
                                        {testRunner.isSaving ? 'Kaydediliyor...' : 'Cevapları Kaydet'}
                                    </Button>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="split-layout">
                            ...
                            {testRunner.data && (
                                <div className="panel">
                                                ...
                                                {auth.isLoggedIn && (
                                                    <select
                                                        ...
                                                        onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                                                            testRunner.setUserAnswers({ ...testRunner.userAnswers, [i + 1]: e.target.value })
                                                        }
                                                    >
                                                        <option value="">-</option>
                                                        {['A', 'B', 'C', 'D', 'E'].map(o => (
                                    ...
                                    {/* Test Score Boxes */}
                                    {testRunner.testScore && testRunner.testScore.hasAnswers && (
                                        <div className="score-boxes">
                                            <div className="score-box dogru">
                                                <span className="score-label">Doğru</span>
                                                <span className="score-value">{testRunner.testScore.dogru}</span>
                                            </div>
                                            <div className="score-box yanlis">
                                                <span className="score-label">Yanlış</span>
                                                <span className="score-value">{testRunner.testScore.yanlis}</span>
                                            </div>
                                            <div className="score-box bos">
                                                <span className="score-label">Boş</span>
                                                <span className="score-value">{testRunner.testScore.bos}</span>
                                            </div>
                                            <div className="score-box net">
                                                <span className="score-label">Net</span>
                                                <span className="score-value">{testRunner.testScore.net}</span>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="test-nav-buttons">
                                        <button
                                            ...
                                            disabled={!books.bookTests.length || lastVisitedTestIndex === null || lastVisitedTestIndex <= 0}
                                            onClick={() => {
                                        ...
                                        <button
                                            className="test-nav-btn"
                                            disabled={!books.bookTests.length || lastVisitedTestIndex === null || lastVisitedTestIndex >= books.bookTests.length - 1}
                                            onClick={() => {
                                                if (lastVisitedTestIndex !== null && lastVisitedTestIndex < books.bookTests.length - 1) {
                            ...
                            <div className="panel">
                                <div className="panel-header">
                                    Video Çözümler
                                    <span className={`status-indicator ${testRunner.videoStatus === 'Tamamlandı' ? 'success' :
                                        testRunner.videoStatus === 'Hata' ? 'error' : 'loading'
                                        }`}>
                                ...
                                <div className="video-list">
                                    {testRunner.videos.map((v, index) => (
                                        <div key={v.q} className="video-item" id={`video-${v.q}`}>
                                            ...
                                            {/* AI CHAT INTERFACE */}
                                            {aiTutor.activeAiQuestion === v.q.toString() && aiTutor.aiContext && (
                                                <div className="mt-4 px-2">
                                                    <AiChat
                                                        initialMessage={aiTutor.aiInitialMessage}
                                                        questionContext={aiTutor.aiContext}
                                                        onClose={() => aiTutor.setActiveAiQuestion(null)}
                                                    />
                                                </div>
                                            )}
                                            ...
                                            <div className="video-nav-buttons">
                                                {index > 0 && (
                                                    <button
                                                        onClick={() =>
                                                            document.getElementById(`video-${testRunner.videos[index - 1].q}`)?.scrollIntoView({ behavior: 'smooth' })
                                                        }
                                                    >
                                                        ← Önceki Soru
                                                    </button>
