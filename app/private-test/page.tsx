  ...
  useEffect(() => {
    ...
    const initFetch = async () => {
      try {
        ...
        const res = await authFetch(`/api/private-test/proxy?testId=${testId}`);
        if (!res.ok) {
          const errorBody = await res.json().catch(() => ({}));
          if (res.status === 401 || res.status === 403) {
            throw new Error(
              errorBody?.error || 'Bu cihaz/tarayıcı private-test erişimi için yetkili değil.'
            );
          }
          throw new Error(errorBody?.error || 'Test verileri çekilemedi.');
        }
        ...
        for (let i = 1; i <= soruSayisi; i++) {
          authFetch(`/api/private-test/video?testId=${testId}&soruId=${i}`)
            .then(async (r) => {
              const payload = await r.json().catch(() => ({}));
              if (!r.ok) {
                if (r.status === 401 || r.status === 403) {
                  setVideoStatus('Bu cihaz/tarayıcı private-test video erişimi için yetkili değil.');
                }
                return null;
              }
              return payload;
