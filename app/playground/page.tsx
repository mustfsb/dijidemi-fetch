...
export default function PlaygroundPage() {
    ...
    try {
        ...
        if (storedSelection) {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
                ...
                // Check for missing images and resolve them
                const resolveImages = async () => {
                        ...
                        if (!q.imageUrl && q.bookId && q.id && q.id.includes('-q')) {
                            try {
                                ...
                                if (data.success && data.imageUrl) {
                                    newQuestions[i].imageUrl = data.imageUrl;
                                    updated = true;
                                }
                            } catch (err) {
                    ...
                    if (updated) {
                         ...
                         // Update session storage too so we don't resolve again on reload
  ...
  const handleSendMessage = async (content: string) => {
    ...
    setHistoryUpdateTrigger(Date.now()); // Update list when user sends
