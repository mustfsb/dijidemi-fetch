...
export default function HomeworkPage() {
  ...
  const handleKttAdd = async (e: React.FormEvent) => {
    ...
    setFetchingKtt(true);
    try {
      // Fetch + save on server-side (admin-authenticated)
      const res = await fetch(`/api/homework/fetch-test`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ testId: newId, save: true }),
      });
      ...
      const { title, answerKey, questionCount } = testData;
      const keyPreview = answerKey ? ` | Cevap: ${answerKey.slice(0, 8)}... (${questionCount} soru)` : '';
      toast.info(`KTT bulundu: ${title}${keyPreview}`);

      toast.success("KTT başarıyla eklendi");
      if (testData.homework) {
        const existingIdx = homeworks.findIndex(h => h.id === testData.homework.id);
        if (existingIdx >= 0) {
          const next = [...homeworks];
          next[existingIdx] = testData.homework;
          setHomeworks(next);
        } else {
          setHomeworks([testData.homework, ...homeworks]);
        }
      } else {
        fetchHomeworks();
      }
