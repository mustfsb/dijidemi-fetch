...
import { cn } from '@/lib/utils'; // Assuming you have a cn utility, if not I'll just use standard template literals or add it. 
// Standard in shadcn/ui projects, usually in lib/utils. I'll check if it exists, otherwise I'll implement inline.
...
const ProgramView: React.FC<ProgramViewProps> = ({ schedule, onToggleTask, onToggleDay, onToggleSubject }) => {
    ...
    return (
        <div className="w-full max-w-[1600px] mx-auto px-6 py-8 flex flex-col gap-8 font-display">
            {/* Header */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div className="flex flex-col gap-2">
                    <h2 className="text-4xl font-black text-white tracking-tight">Haftalık Ödevler</h2>
                    <p className="text-zinc-400 text-base">Tüm derslerdeki ödev ilerlemeni takip et.</p>
                </div>
                ...
                <div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-surface-dark border border-border-dark rounded-xl p-5 flex flex-col justify-between h-32 group hover:border-zinc-700 transition-colors">
                        <div className="flex justify-between items-start">
                            <span className="text-zinc-400 text-sm font-medium">Toplam Görev</span>
                            <span className="material-symbols-outlined text-zinc-600">list_alt</span>
                        </div>
                        <div>
                    ...
                    <div className="bg-surface-dark border border-border-dark rounded-xl p-5 flex flex-col justify-between h-32 group hover:border-zinc-700 transition-colors">
                        <div className="flex justify-between items-start">
                            <span className="text-zinc-400 text-sm font-medium">Tamamlanan</span>
                            <span className="material-symbols-outlined text-zinc-600">check_circle</span>
                        </div>
                        <div>
                    ...
                    <div className="bg-surface-dark border border-border-dark rounded-xl p-5 flex flex-col justify-between h-32 group hover:border-zinc-700 transition-colors">
                        <div className="flex justify-between items-start">
                            <span className="text-zinc-400 text-sm font-medium">Kalan</span>
                            <span className="material-symbols-outlined text-zinc-600">schedule</span>
                        </div>
                        <div>
                    ...
                    */}
                    <div className="bg-surface-dark border border-border-dark rounded-xl p-5 flex flex-col justify-between h-32 group hover:border-zinc-700 transition-colors">
                        <div className="flex justify-between items-start">
                            <span className="text-zinc-400 text-sm font-medium">Odak</span>
                                            ...
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <div className="w-16 h-1 bg-zinc-800 rounded-full overflow-hidden">
                                                        <div
                                                            className={`h-full ${subjectProgress === 100 ? 'bg-primary' : 'bg-zinc-500'}`}
                                                            style={{ width: `${subjectProgress}%` }}
                                                        ></div>
                                                    </div>
                                                    <span className="text-[10px] text-zinc-500">{subjectProgress}%</span>
                                                </div>
                                            </div>
                                        </td>
                                            ...
                                            return (
                                                <td key={dayName} className="p-2 align-top border-r border-border-dark/30">
                                                    <div
                                                        ...
                                                        `}
                                                    >
                                                        <span className={`text-xs font-bold leading-tight ${dayTask.completed ? 'text-white line-through opacity-90' : 'text-zinc-300 group-hover/cell:text-white font-normal'}`}>
                                                            {dayTask.name} <br /> <span className="text-[10px] opacity-70">{dayTask.pageRange}</span>
                                                        </span>
                                                        <div className={`flex justify-end transition-opacity ${dayTask.completed ? '' : 'opacity-0 group-hover/cell:opacity-100'}`}>
                                                            <span className="material-symbols-outlined text-white text-[18px]">
                                                                {dayTask.completed ? 'check_circle' : 'radio_button_unchecked'}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </td>
